...
public class TestQueries {
	...
	public static void hostsConntectedTo(Jedis jedis, String ip) {
		...
		resultCountersSource.addAll(
				jedis.lrange(groupKeys.get(0).toString().replace(",", "")  //remove the commas
				    .replace("[", "")  //remove the right bracket
				    .replace("]", ""), 0, -1 )
		);
		resultCountersSource.retainAll(
				jedis.lrange(groupKeys.get(1).toString().replace(",", "")  //remove the commas
				    .replace("[", "")  //remove the right bracket
				    .replace("]", ""), 0, -1 )
		);
		
		//Destiation get all IPS that are in the port and addr of source
		resultCountersDestination.addAll(
				jedis.lrange(groupKeys.get(2).toString().replace(",", "")  //remove the commas
				    .replace("[", "")  //remove the right bracket
				    .replace("]", ""), 0, -1 )
		);
		resultCountersDestination.retainAll(
				jedis.lrange(groupKeys.get(3).toString().replace(",", "")  //remove the commas
				    .replace("[", "")  //remove the right bracket
				    .replace("]", ""), 0, -1 )
		);
		
		//Add Source and Destination to a single set -->remove automatically duplicates
		resultsCountersAll.addAll(resultCountersDestination);
		resultsCountersAll.addAll(resultCountersSource);
		
		System.out.println(resultsCountersAll);
